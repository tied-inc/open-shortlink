...
```
クライアント → POST /api/links (OAuth access token)
  → OAuthProvider がアクセストークンを検証（401 / 503）
  → URL バリデーション
...
- **API エンドポイント** (`/api/*`): OAuth 2.1 アクセストークン
- **MCP エンドポイント** (`/mcp`): OAuth 2.1 アクセストークン

`@cloudflare/workers-oauth-provider` が `/api` と `/mcp` の前段でアクセストークン
を検証する。ユーザー本人性は Worker では扱わず、**外部 IdP に委任**する
（Cloudflare Access か任意の OpenID Connect プロバイダを選択）。詳細は
[セキュリティポリシー](./security)。
...
open-shortlink/
├── src/
│   ├── index.ts              # OAuthProvider で /api, /mcp を保護
│   ├── app.ts                # defaultHandler (redirect, /authorize, /oauth/callback)
│   ├── routes/
│   │   ├── redirect.ts       # GET /:slug → 302
│   │   └── api.ts            # REST API (CRUD + 分析) — OAuth の背後
│   ├── mcp/
│   │   ├── server.ts         # JSON-RPC / Streamable HTTP — OAuth の背後
│   │   └── tools.ts          # MCP ツール定義
│   ├── oauth/
│   │   ├── authorize.ts      # /authorize と /oauth/callback
│   │   └── idp/
│   │       ├── index.ts      # IdP モード選択と allowlist
│   │       ├── access.ts     # Cloudflare Access JWT 検証
│   │       └── oidc.ts       # 汎用 OIDC discovery + PKCE + ID token 検証
│   ├── analytics/
