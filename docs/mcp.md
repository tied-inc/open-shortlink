...
| `/.well-known/oauth-protected-resource` | 保護リソースメタデータ (RFC 9728) |
| `/authorize` | 運用者が構成した IdP にサインインを委譲 |
| `/oauth/callback` | OIDC モード時に上流 IdP からの戻り先 |
| `/token` | トークン交換 |
| `/register` | 動的クライアント登録 (RFC 7591) |

`/authorize` の挙動は運用者が構成した IdP に依存する:

- **Cloudflare Access モード**: Access が付与する JWT を検証 → 許可なら
  そのまま OAuth 認可を完了
- **汎用 OIDC モード**: PKCE + state + nonce を生成し、上流 IdP の
  authorization endpoint へ 302 リダイレクト。上流でのサインイン後
  `/oauth/callback` に戻り、ID token を JWKS で検証してから OAuth
  認可を完了
...
初回接続時にブラウザが開き、Worker 側で構成された IdP（Cloudflare Access
か任意の OpenID Connect プロバイダ）のサインイン画面にリダイレクトされる。
許可された email / sub でサインインすると OAuth フローが自動的に完了し、
Claude Desktop のチャット画面右下のツールアイコンに `shortlink` が
表示される。
